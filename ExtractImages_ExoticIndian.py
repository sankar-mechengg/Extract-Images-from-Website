...
def extract_link_csv(file):
    ...
    # Replace a string in the link with another string
    for index, row in df.iterrows():
        url = row[0]
        url = url.replace("/thumbnails/t400x300/", "/original/")
        row[0] = url
...
def main(url):
    ...
    filename = website_name + dt + ".csv"

    page_content = get_content_from_url(url)
    image_urls = parse_image_urls(
        content=page_content, classes="product-imgarea", location="img", source="src")
...
if __name__ == "__main__":
    ...
    # url = "https://www.exoticindiaart.com/paintings/collections/"
    # main(url)
